...
module.exports = class Api {
  ...
  async getChecks() {
    ...
    // get checks details:
    return await Bluebird.map(data, async (check) => {
      const checkDetails = await this.getCheck(check.id);
      // add "region" to object
      const region = _.get(checkDetails, 'probe_filters[0]', '').match(/^region: (\w+)$/);
      checkDetails.region = region && region.length ? region[1] : 'default';
      return checkDetails;
